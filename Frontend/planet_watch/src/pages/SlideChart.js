...
function SlideChart(){
  ...
  useEffect(()=>{
    ...
    axios.get('http://127.0.0.1:8000/getChartslide/').then((res)=>{
      const dailyRainfall = res.data.daily.et0_fao_evapotranspiration;
   ...
   {FloodIncident !== 0 && (
    <ResponsiveContainer width="100%" height={300} >
      ...
      >
        ...
        <Tooltip />
        <Legend />
        <Line type="monotone" dataKey="Evapotranspiration" stroke="#8884d8" activeDot={{ r: 8 }} />
      </LineChart>
    </ResponsiveContainer>
   )}
     </div>
